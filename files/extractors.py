...
TAIL_CUTOFF_PATTERNS = (
    ...
    # 「※本文由今周刊授權刊載，未經同意禁止轉載。」udn 轉載他家內容時會掛這行，
    # 後面接的是「你可能也會感興趣》」加一串別則新聞標題。
    # 一定要連著「未經同意」一起比對：光看「禁止」或「轉載」會誤砍真的內文
    # （0806 那天有一段寫「遭世界銀行禁止參與相關招標」，那是新聞本身）。
    re.compile(r"^※?\s*本文由.{0,20}授權(刊載|轉載)"),
    re.compile(r"未經同意.{0,4}(禁止|不得)轉載"),
    re.compile(r"^你可能也會(感興趣|喜歡)"),
    # 「※更多精彩報導，詳見《商業周刊》網站。」udn 轉載商周稿子時掛在文末的導流句。
    # 兩條分開寫：轉載來源不只商周，一條認 ※ 開頭的「更多…報導」，
    # 一條認「詳見《…》網站」這個句型。不敢只用「更多…報導」開頭就擋，
    # 真的內文也可能出現「更多相關報導指出…」，而這裡一擋就是砍到文末。
    re.compile(r"^※\s*更多.{0,10}報導"),
    re.compile(r"詳見《.{0,15}》(網站|網頁)"),
)
